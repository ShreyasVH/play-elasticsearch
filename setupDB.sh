...
RESPONSE=$(curl -u "$ELASTIC_USERNAME:$ELASTIC_PASSWORD" -X POST "https://$ELASTIC_IP_HTTP:$ELASTIC_PORT_HTTP/_security/api_key" -H "Content-Type: application/json" -d '{ "name": "'$ELASTIC_INDEX_BOOKS'", "expiration": "90d", "role_descriptors": { "cluster_writer_role": { "cluster": [ "monitor" ], "index": [ { "names": ["'$ELASTIC_INDEX_BOOKS'"], "privileges": [ "all" ] } ] } } }')
